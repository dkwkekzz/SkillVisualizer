...
namespace PLH {
	namespace Tools
	{
		inline void XTrace(const char* fmt, ...)
		{
			va_list args;
			va_start(args, fmt);
#if defined(_DEBUG) && !defined(PLH_HIDE_DEBUG_MESSAGES)
...
uint_fast32_t PLH::AbstractDetour::CalculateLength(uint8_t* Src, uint_fast32_t NeededLength)
{
	...
	for (uint_fast32_t i = 0; i < InstructionCount && !BigEnough; i++)
	{
		...
		if (strcmp(CurIns->mnemonic, "ret") == 0 || strcmp(CurIns->mnemonic, "jmp") == 0 || strcmp(CurIns->mnemonic, "call") == 0)
			break;
	}
...
bool PLH::X64Detour::Hook()
{
	...
	constexpr int hookingLen = jmpLen;
	//constexpr int hookingLen = jmpLen;
	...
	if (m_hkLength == 0)
	{
		PostError(RuntimeError(RuntimeError::Severity::Critical, "PolyHook x64Detour: Function to small to hook"));
		FreeTrampoline();
		return false;
	...
	if (UseRelativeJmp)
	{
		...
		*(uintptr_t*)&m_Trampoline[m_hkLength + jmpLen] = (uintptr_t)m_hkDest; //Write the address into memory at [RIP+Displacement]
		...
		HookSize = hookingLen;
		//WriteCall((uintptr_t)m_hkSrc, (uintptr_t)m_hkMid);
...
void PLH::X64Detour::WriteCall(const uintptr_t Destination, const uintptr_t CallDestination)
{
	// with call!
	uint8_t call_detour[] = {
