...
void SymbolTable::EnumerateAllByAddr(std::vector<std::wstring>& namelist, std::vector<ULONGLONG>& ptrlist)
{
	HRESULT hr;
	...
	if (pSym->get_relativeVirtualAddress(&rvaLast) == S_OK)
	{
		...
		do
		{
			...
			else
			{
				...
				//	//|| (!StrStr(name, L"powerworld::")&& !StrStr(name, L"STATIC::"))
				//	//|| !StrStr(name, L"_testdummy") 
				//	)
				//{
				//	pSym = 0;
				//	celt = 0;
			...
			if (++testcount % 10000 == 0)
				std::wcout << L"count: " << testcount << std::endl;
